...
// In UDP_SERVER mode the CH9120 listens and typically uses broadcast
// (255.255.255.255) until a peer is established For UDP_CLIENT mode, the
// address can be used directly
UCHAR CH9120_Mode =
    UDP_CLIENT; // TCP_SERVER / TCP_CLIENT / UDP_SERVER / UDP_CLIENT
...
void DEV_Delay_us(UDOUBLE xus) { delayMicroseconds(xus); }
...
******************************************************************************/
void CH9120_init(UCHAR local_ip[4], UCHAR gateway_ip[4], UCHAR subnet_mask[4],
                 UCHAR remote_ip[4], UWORD local_port, UWORD remote_port) {
  Serial.begin(115200);
  delay(1000);
